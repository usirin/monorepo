...
export class TransactionIngestor extends Effect.Service<TransactionIngestor>()(
  "TransactionIngestor",
  {
    ...
    scoped: Effect.gen(function* () {
      ...
      const ingestTransactions = Effect.fn(
        "TransactionIngestor.ingestTransactions",
      )(function* ({ startDate }: { startDate: number }) {
        yield* Effect.log(
          `Ingesting transactions from ${new Date(startDate).toISOString()}`,
        );
