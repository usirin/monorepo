...
export const split = (tree: Tree, index: number, orientation: Orientation): Tree => {
	...
	if (node === tree.root) {
		const newRoot = createNode({data: node.data, orientation});
	...
	if (orientation !== parent.orientation) {
		const newParent = createNode({data: node.data, orientation});
